...
const Employees = () => {
  ...
  const { session, employee_edit_id, employee_delete_id } = state;
  ...
  const HANDLE_CODES = () => {
    ...
    dispatch({
      type: "EMPLOYEE_GENERATE_ID",
      payload: generateUniqueID.toString(),
    });
  };
  ...
  const HANDLE_BUTTON = (username) => {
    if (useDelete) {
      setUseDelete(false);
      dispatch({ type: "EMPLOYEE_DELETE_ID", payload: "" });
    } else {
      setUseDelete(true);
      dispatch({ type: "EMPLOYEE_DELETE_ID", payload: username });
    }
  };
      ...
      <Wrapper>
        ...
        <Items>
          <Link href="/employees/add">
            <button onClick={HANDLE_CODES}>ADD NEW</button>
          </Link>
        </Items>
          ...
          <table>
            ...
            <tbody>
              {data &&
                data.map((data) => (
                  <tr key={data._id}>
                    <td style={{ fontWeight: 600 }}>{data.id}</td>
                    <td>
                    ...
                    <td>
                      ...
                      <button onClick={() => HANDLE_BUTTON(data.username)}>
                        <DeleteIcon />
                      </button>
                    </td>
                  </tr>
